<!-- To see all peer to peer loan records. loan records are gotten from peer_reords table -->
<?php 
...
<body>
	...
	<div class="wrapper"> <!-- opening Div Wrapper Tag -->
        ...
        <li class="nav_item">
          ...
          <li class="nav_item_active">
		        <a class="nav_link" href="loanrecords.php">
		          loan Request(s)
		        </a>
		      </li>

          <li class="nav_item">
		        <a class="nav_link" href="directrequest.php">
		          Direct Requests
		        </a>
		      </li>
		    </ul>
				        ...
				        <ul class="profile_drop_menu" > <!-- opening Profile DropDown Menu Tag -->
				        	<li class="profiledrop">
				            	...
				            	<!--	<li class="dropdown_item"> <a class="" href="#">Profile</a> </li>
				            		...
				            		<li class="dropdown_item"> <a class="" href="loanrecords.php?logout='1'">Log out</a> </li>    
            ...
            <div class="totver" >  <!-- opening Total Verified Tag , also by verification cards-->
                ...
                <div class="nav_tab">
                  <p class=""> 
                    <?php
                      ...
                      $query = "SELECT COUNT(*) from loan_record where status IS NULL and granted_entity_username IS NULL";
              ...
              <div class="card_body">
                <div class="tab-pane active">  <!-- opening Users Tag -->
                    ...
                    $users = "SELECT count(id) FROM loan_record where status IS NULL and granted_entity_username IS NULL ORDER by id ASC";
                    ...
                    if ($page) {
                    echo '<a href="loanrecords.php"><button><<</button></a>';// Button for first page [<<]
                    echo '<a href="loanrecords.php?page='.$page_down.'"><button><</button></a>';// Button for previous page [<]
                    } 

                    for ($i=1;$i<=$pages_total;$i++) {
                    if(($i==$page+1)) {
                    echo '<a href="loanrecords.php?page='.$i.'"><button class="active">'.$i.'</button></a>';// Button for active page, underlined using 'active' class
                    }
                    ...
                    echo '<a href="loanrecords.php?page='.$i.'"><button>'.$i.'</button></a>'; }
                    } 

                    if (($page + 1) != $pages_total) {
                    echo '<a href="loanrecords.php?page='.$page_up.'"><button>></button></a>';// Button for next page [>]
                    echo '<a href="loanrecords.php?page='.$pages_total.'"><button>>></button></a>';// Button for last page [>>]
                    }
                  ...
                  <table class="card_table">
                    ...
                    <tbody>
                      <?php
                        $mysqli = new mysqli("localhost:3306", "emeka", "emeka123", "loan") or die ("connection failed: " .$mysqli->connect_error());
                        // $query = $mysqli->query("SELECT id,recipient_username,amount,description from peer_record where status IS NULL and sender_username IS NULL ORDER by id ASC LIMIT $offset, $per_page");
                        $query = $mysqli->query("SELECT loan_record.id,loan_record.recipient_username,loan_record.amount,loan_record.description,users.ratings,users.username from loan_record INNER JOIN users ON loan_record.recipient_username=users.username WHERE status IS NULL and granted_entity_username IS NULL ORDER by id ASC LIMIT $offset, $per_page");
  
                        while($row = mysqli_fetch_array($query))
                        {
                          Print "<tr>";
                          Print '<td align="center">'. $row['recipient_username'] . "</td>";
                          Print '<td align="center">'. $row['ratings'] . "</td>";
                          Print '<td align="center">'. $row['amount'] . "</td>";
                          Print '<td align="center">'. $row['description'] . "</td>";
                          Print '<td align="center"><a href="#" onclick="sendfund('.$row['id'].')"> send fund</a> </td>';
                          Print "</tr>";
                        }
                      ?>        
                    </tbody>
  ...
  <script>
    function sendfund(id)
    {
    var r=confirm("Are you sure you want to send fund to person?");
    if (r==true)
      {
        window.location.assign("sendfund.php?id=" + id);
      }
    }
  </script>
