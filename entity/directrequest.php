	...
	<div class="wrapper"> <!-- opening Div Wrapper Tag -->
		...
		<div class="main_panel"> <!-- opening Div Main Panel Tag -->
				        ...
				        <ul class="profile_drop_menu" > <!-- opening Profile DropDown Menu Tag -->
				        	<li class="profiledrop">
				            	...
				            	<!--	<li class="dropdown_item"> <a class="" href="#">Profile</a> </li>
				            		...
				            		<li class="dropdown_item"> <a class="" href="directrequest.php?logout='1'">Log out</a> </li>    
            ...
            <div class="totver" >  <!-- opening Total Verified Tag , also by verification cards-->
              <div class="card_head">
                <p class=""> My Recieved Request(s)</p>
              </div>
              <div class="nav_head">
                <div class="nav_tab">
                  <p class=""> 
                    ...
                    if (isset($_SESSION['entity'])) {				
                        ...
                        $my_username = $entity['username'];
                    }
                      $mysqli = new mysqli("localhost:3306", "emeka", "emeka123", "loan") or die ("connection failed: " .$mysqli->connect_error());
                      $query = "SELECT COUNT(*) from loan_record where granted_entity_username='$my_username' AND status IS NULL";
              ...
              <div class="card_body">
                <div class="tab-pane active">  <!-- opening Users Tag -->
                    ...
                    $users = "SELECT count(id) FROM loan_record where granted_entity_username='$my_username' AND status IS NULL ORDER by id ASC";
                    ...
                    if ($page) {
                    echo '<a href="directrequest.php"><button><<</button></a>';// Button for first page [<<]
                    echo '<a href="directrequest.php?page='.$page_down.'"><button><</button></a>';// Button for previous page [<]
                    } 

                    for ($i=1;$i<=$pages_total;$i++) {
                    if(($i==$page+1)) {
                    echo '<a href="directrequest.php?page='.$i.'"><button class="active">'.$i.'</button></a>';// Button for active page, underlined using 'active' class
                    }
                    ...
                    echo '<a href="directrequest.php?page='.$i.'"><button>'.$i.'</button></a>'; }
                    } 

                    if (($page + 1) != $pages_total) {
                    echo '<a href="directrequest.php?page='.$page_up.'"><button>></button></a>';// Button for next page [>]
                    echo '<a href="directrequest.php?page='.$pages_total.'"><button>>></button></a>';// Button for last page [>>]
                    }
                    ...
                    <tbody>
                      <?php
                        ...
                        if (isset($_SESSION['entity'])) {				
                            ...
                            $user = $_SESSION['entity'];
                            $my_username = $user['username'];
                        }
                        // Query to get pull ratings from users table where username in both table are same
                        $query = $mysqli->query("SELECT loan_record.id,loan_record.recipient_username,loan_record.granted_entity_username,loan_record.amount,loan_record.description,users.ratings,users.username from loan_record INNER JOIN users ON loan_record.recipient_username=users.username WHERE loan_record.granted_entity_username='$my_username'  AND status IS NULL ORDER by id ASC LIMIT $offset, $per_page");
